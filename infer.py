...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-path", default="FedDQN/result/3nodes/next_or_not/global/q_net_best.pt", help="Path to trained QNetwork weights.")
    parser.add_argument("--sumo-cfg", default="Scenario/3nodes/env/vehicle.sumocfg", help="SUMO config file (.sumocfg).")
    parser.add_argument("--net-file", default="Scenario/3nodes/env/3nodes.net.xml", help="SUMO net file used to build the graph.")
    parser.add_argument("--tls-action-type", default="next_or_not", help="Traffic-light action type used during training.")
    parser.add_argument("--trip-info", default="deploy_3nodes.tripinfo.xml", help="Optional SUMO tripinfo output path.")
    parser.add_argument("--num-seconds", type=int, default=500, help="Simulation horizon in seconds.")
    parser.add_argument("--log-path", default='deploy_3nodes.log.monitor.csv', help="Path to monitor log output.")
...
def prepare_env(node_id_list, sumo_cfg: str, net_file: str, trip_info: str, num_seconds: int, use_gui: bool, log_path: Path, tls_action_type: str):
...
def resolve_model_path(model_path_arg: str) -> Path:
    ...
    if model_path.name == "q_net_best.pt":
        candidates = sorted(model_path.parent.glob("q_net_round*.pt"))
        if candidates:
            fallback = candidates[-1]
            print(f"Không tìm thấy {model_path}; dùng checkpoint mới nhất: {fallback}")
            return fallback
...
def main():
    ...
    action_dim = 2 if args.tls_action_type == "next_or_not" else max(1, phase_dim * 2)
