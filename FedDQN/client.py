...
class DQNClient:
    """Trainer for one TLS id, using either a local k-hop view or the full graph."""
    ...
    def __init__(
        ...
        if client_scope == "global":
            self.graph_view = self._build_global_graph_view(graph=graph)
        elif client_scope == "local":
            self.graph_view = self._build_local_graph_view(graph=graph, hop_k=neighbor_hop_k)
        else:
            raise ValueError(f"Unsupported client_scope='{client_scope}'. Expected 'global' or 'local'.")
    ...
    def _build_global_graph_view(self, graph: GraphSpec):
